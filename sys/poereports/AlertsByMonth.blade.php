    ...
    <div class="card-body py-4">
        ...
        <div class="d-flex flex-stack flex-wrap mb-8">
            <div class="d-flex flex-wrap align-items-center">
                ...
                <!--begin::Classification Filter-->
                <div class="w-200px me-4 my-2">
                    <select id="classification-filter" class="form-select form-select-solid" data-control="select2"
                        data-placeholder="Select Classification">
                        <option value="">All Classifications</option>
                        @foreach ($classifications as $classification)
                            <option value="{{ $classification }}"
                                {{ $selectedClassification == $classification ? 'selected' : '' }}>{{ $classification }}
                            </option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <select id="month-filter" class="form-select form-select-solid" data-control="select2"
                        ...
                        @for ($i = 1; $i <= 12; $i++)
                            <option value="{{ $i }}" {{ $selectedMonth == $i ? 'selected' : '' }}>
                                {{ date('M', mktime(0, 0, 0, $i, 1)) }}</option>
                        @endfor
                    </select>
                </div>
                <!--end::Month Filter-->
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const alertVolumeData = Array(12).fill(0); // Initialize with zero for each month

        // Populate alertVolumeData based on chartData from the backend
        chartData.forEach(item => {
            const monthIndex = item.month - 1; // Convert month to array index
            alertVolumeData[monthIndex] = item.alert_volume;
        });
        ...
        var options = {
            ...
            title: {
                text: 'Monthly Alerts Volume by POE'
            },
            labels: monthNames,
            xaxis: {
                type: 'category',
        ...
        document.getElementById('apply-filters').addEventListener('click', function() {
            ...
            url.searchParams.set('classification', document.getElementById('classification-filter')
                .value);
